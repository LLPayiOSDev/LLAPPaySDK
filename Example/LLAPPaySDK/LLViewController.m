...
- (IBAction)checkSupportStatus:(id)sender {
    ...
    switch (status) {
        case kLLAPPayDeviceSupport: [self llAlertWithTitle:@"提示" andMsg:@"完全支持"]; break;
        case kLLAPPayDeviceNotSupport: [self llAlertWithTitle:@"提示" andMsg:@"机型不支持，或者系统版本太低"]; break;
        case kLLAPPayDeviceVersionTooLow: [self llAlertWithTitle:@"提示" andMsg:@"设备无法支持银联卡支付，需要iOS9."
                                                                                  @"2以上"]; break;
        case kLLAPPayDeviceNotBindChinaUnionPayCard: [self llAlertWithTitle:@"提示" andMsg:@"用户未绑卡"]; break;
        default: break;
    }
}
...
- (NSDictionary *)createOrder {
    ...
    order[@"risk_item"] = [LLPayUtil jsonStringOfObj:@{
        @"user_info_dt_register" : @"20131030122130",
        @"cinema_name" : @"大电影院",
